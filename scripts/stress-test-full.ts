...
class Stats {
  ...
  printProgress(): void {
    ...
    write(
      `  [${elapsed}s] ${String(this.totalOps)} ops (${opsPerSec}/s), ${String(this.totalErrors)} err, ${String(this.coveredTools)} tools hit, heap: ${memMb}MB, ${remaining}s left`,
    );
...
const g01: OpFn = (ctx) =>
  op(ctx, "G:list_files_in_vault", () =>
    ctx.client.listFilesInVault().then(() => {}),
  );

// #2 list_files_in_dir
const g02: OpFn = (ctx) =>
  op(ctx, "G:list_files_in_dir", async () => {
    try {
      await ctx.client.listFilesInDir(".");
    } catch {
      /* dir may not exist */
    }
  });
...
const g03: OpFn = (ctx) =>
  op(ctx, "G:get_file_contents", () =>
    ctx.client.getFileContents(pick(ctx.files), "markdown").then(() => {}),
  );

// #4 put_content
const g04: OpFn = (ctx) =>
  op(ctx, "G:put_content", () =>
    ctx.client.putContent(pick(ctx.files), `# Updated\n\nAt ${Date.now()}\n`),
  );

// #5 append_content
const g05: OpFn = (ctx) =>
  op(ctx, "G:append_content", () =>
    ctx.client.appendContent(pick(ctx.files), `\nAppend ${Date.now()}\n`),
  );

// #6 patch_content — Target uses :: delimiter for heading hierarchy
const g06: OpFn = (ctx) =>
  op(ctx, "G:patch_content", async () => {
    const file = pick(ctx.files);
    await ctx.client.putContent(
      file,
      `# PatchRoot\n\n## PatchChild\n\nContent here.\n`,
    );
...
const g08: OpFn = (ctx) =>
  op(ctx, "G:search_replace", async () => {
    ...
    if (typeof content === "string") {
      await ctx.client.putContent(
        file,
        content.replace(/OLD_TOKEN_\d+/, `NEW_TOKEN_${Date.now()}`),
      );
    }
  });
...
// #9 get_active_file
const g09: OpFn = (ctx) =>
  op(ctx, "G:get_active_file", () =>
    ctx.client.getActiveFile("markdown").then(() => {}),
  );

// #10 put_active_file
const g10: OpFn = (ctx) =>
  op(ctx, "G:put_active_file", () =>
    ctx.client.putActiveFile(`# Active\n\nStress ${Date.now()}\n`),
  );

// #11 append_active_file
const g11: OpFn = (ctx) =>
  op(ctx, "G:append_active_file", () =>
    ctx.client.appendActiveFile(`\nActive append ${Date.now()}\n`),
  );

// #12 patch_active_file — Target uses :: delimiter
const g12: OpFn = (ctx) =>
  op(ctx, "G:patch_active_file", async () => {
    await ctx.client.putActiveFile(
      `# ActiveRoot\n\n## ActiveChild\n\nContent.\n`,
    );
...
// #14 list_commands
const g14: OpFn = (ctx) =>
  op(ctx, "G:list_commands", () => ctx.client.listCommands().then(() => {}));

// #15 execute_command (safe: toggle-sidebar)
const g15: OpFn = (ctx) =>
  op(ctx, "G:execute_command", () =>
    ctx.client.executeCommand("app:toggle-left-sidebar"),
  );

// #16 open_file
const g16: OpFn = (ctx) =>
  op(ctx, "G:open_file", () => ctx.client.openFile(pick(ctx.files)));

// #17 simple_search
const g17: OpFn = (ctx) =>
  op(ctx, "G:simple_search", () =>
    ctx.client
      .simpleSearch(pick(["stress", "test", "updated", "content"]))
      .then(() => {}),
  );

// #18 complex_search (JsonLogic glob)
const g18: OpFn = (ctx) =>
  op(ctx, "G:complex_search", () =>
    ctx.client
      .complexSearch({ glob: [{ var: "path" }, `${STRESS_PREFIX}*.md`] })
      .then(() => {}),
  );

// #19 dataview_search
const g19: OpFn = (ctx) =>
  op(ctx, "G:dataview_search", () =>
    ctx.client.dataviewSearch('TABLE file.name FROM "" LIMIT 5').then(() => {}),
  );

// #20 get_periodic_note
const g20: OpFn = (ctx) =>
  op(ctx, "G:get_periodic_note", () =>
    ctx.client.getPeriodicNote("daily", "markdown").then(() => {}),
  );

// #21 put_periodic_note
const g21: OpFn = (ctx) =>
  op(ctx, "G:put_periodic_note", () =>
    ctx.client.putPeriodicNote("daily", `# Daily\n\nStress ${Date.now()}\n`),
  );

// #22 append_periodic_note
const g22: OpFn = (ctx) =>
  op(ctx, "G:append_periodic_note", () =>
    ctx.client.appendPeriodicNote("daily", `\nAppend ${Date.now()}\n`),
  );

// #23 patch_periodic_note — use :: delimiter
const g23: OpFn = (ctx) =>
  op(ctx, "G:patch_periodic_note", async () => {
...
const g25: OpFn = (ctx) =>
  op(ctx, "G:get_periodic_note_for_date", () =>
    ctx.client
      .getPeriodicNoteForDate("daily", 2026, 3, 15, "markdown")
      .then(() => {}),
  );

// #26 put_periodic_note_for_date
const g26: OpFn = (ctx) =>
  op(ctx, "G:put_periodic_note_for_date", () =>
    ctx.client.putPeriodicNoteForDate(
      "daily",
      2026,
      3,
      15,
...
const g27: OpFn = (ctx) =>
  op(ctx, "G:append_periodic_note_for_date", () =>
    ctx.client.appendPeriodicNoteForDate(
      "daily",
      2026,
      3,
      15,
...
const g28: OpFn = (ctx) =>
  op(ctx, "G:patch_periodic_note_for_date", async () => {
    await ctx.client.putPeriodicNoteForDate(
      "daily",
      2026,
      3,
      15,
    ...
    await ctx.client.patchPeriodicNoteForDate(
      "daily",
      2026,
      3,
      15,
...
const g30: OpFn = (ctx) =>
  op(ctx, "G:get_server_status", () =>
    ctx.client.getServerStatus().then(() => {}),
  );

// #31 batch_get_file_contents — via getFileContents in a loop (client method)
const g31: OpFn = (ctx) =>
  op(ctx, "G:batch_get_file_contents", async () => {
    const batch = ctx.files.slice(0, 5);
    await Promise.all(
      batch.map((f) => ctx.client.getFileContents(f, "markdown")),
    );
  });

// #32 get_recent_changes — derived from vault listing + stat.mtime (uses cache)
const g32: OpFn = (ctx) =>
  op(ctx, "G:get_recent_changes", async () => {
...
const g33: OpFn = (ctx) =>
  op(ctx, "G:get_recent_periodic_notes", async () => {
...
// #35 get_backlinks
const g35: OpFn = (ctx) =>
  op(ctx, "G:get_backlinks", async () => {
    ctx.cache.getBacklinks(pick(ctx.files));
  });

// #36 get_vault_structure
const g36: OpFn = (ctx) =>
  op(ctx, "G:get_vault_structure", async () => {
...
const g37: OpFn = (ctx) =>
  op(ctx, "G:get_note_connections", async () => {
    const file = pick(ctx.files);
...
const c01: OpFn = (ctx) =>
  op(ctx, "C:vault:list", () => ctx.client.listFilesInVault().then(() => {}));
const c01b: OpFn = (ctx) =>
  op(ctx, "C:vault:get", () =>
    ctx.client.getFileContents(pick(ctx.files), "markdown").then(() => {}),
  );
const c01c: OpFn = (ctx) =>
  op(ctx, "C:vault:put", () =>
    ctx.client.putContent(pick(ctx.files), `# C-Put ${Date.now()}\n`),
  );
const c01d: OpFn = (ctx) =>
  op(ctx, "C:vault:append", () =>
    ctx.client.appendContent(pick(ctx.files), `\nC-Append ${Date.now()}\n`),
  );

// C2: active_file (get, put, append)
const c02: OpFn = (ctx) =>
  op(ctx, "C:active_file:get", () =>
    ctx.client.getActiveFile("markdown").then(() => {}),
  );
const c02b: OpFn = (ctx) =>
  op(ctx, "C:active_file:put", () =>
    ctx.client.putActiveFile(`# C-Active ${Date.now()}\n`),
  );

// C3: commands (list, execute)
const c03: OpFn = (ctx) =>
  op(ctx, "C:commands:list", () => ctx.client.listCommands().then(() => {}));

// C4: open_file
const c04: OpFn = (ctx) =>
  op(ctx, "C:open_file", () => ctx.client.openFile(pick(ctx.files)));

// C5: search (simple, jsonlogic, dataview)
const c05: OpFn = (ctx) =>
  op(ctx, "C:search:simple", () =>
    ctx.client.simpleSearch("stress").then(() => {}),
  );
const c05b: OpFn = (ctx) =>
  op(ctx, "C:search:jsonlogic", () =>
    ctx.client
      .complexSearch({ glob: [{ var: "path" }, "*.md"] })
      .then(() => {}),
  );
const c05c: OpFn = (ctx) =>
  op(ctx, "C:search:dataview", () =>
    ctx.client.dataviewSearch('TABLE file.name FROM "" LIMIT 3').then(() => {}),
  );

// C6: periodic_note (get, put, append)
const c06: OpFn = (ctx) =>
  op(ctx, "C:periodic_note:get", () =>
    ctx.client.getPeriodicNote("daily").then(() => {}),
  );
const c06b: OpFn = (ctx) =>
  op(ctx, "C:periodic_note:put", () =>
    ctx.client.putPeriodicNote("daily", `# C-Daily ${Date.now()}\n`),
  );

// C7: status
const c07: OpFn = (ctx) =>
  op(ctx, "C:status", () => ctx.client.getServerStatus().then(() => {}));

// C8: batch_get
const c08: OpFn = (ctx) =>
  op(ctx, "C:batch_get", async () => {
    await Promise.all(
      ctx.files.slice(0, 3).map((f) => ctx.client.getFileContents(f, "json")),
    );
  });

// C9: recent (changes)
const c09: OpFn = (ctx) =>
  op(ctx, "C:recent:changes", async () => {
    const notes = ctx.cache.getAllNotes();
...
// C10: configure (show)
const c10: OpFn = (ctx) =>
  op(ctx, "C:configure:show", async () => {
    void ctx.cache.noteCount;
  });

// C11: vault_analysis (backlinks, connections, structure, refresh)
const c11: OpFn = (ctx) =>
  op(ctx, "C:vault_analysis:backlinks", () => {
    ctx.cache.getBacklinks(pick(ctx.files));
    return Promise.resolve();
  });
const c11b: OpFn = (ctx) =>
  op(ctx, "C:vault_analysis:structure", () => {
    ctx.cache.getMostConnectedNotes(5);
    ctx.cache.getOrphanNotes();
    return Promise.resolve();
  });
const c11c: OpFn = (ctx) =>
  op(ctx, "C:vault_analysis:refresh", () => ctx.cache.refresh());
...
async function main(): Promise<void> {
  ...
  write(
    `  ${String(filePool.length)} files seeded, cache: ${String(cache.noteCount)} notes`,
  );
  write("");
  ...
  try {
    await Promise.all(
      Array.from({ length: CONCURRENCY }, () => worker(ctx, deadline)),
    );
  } finally {
  ...
  write(
    `  Coverage: ${String(stats.coveredTools)}/${String(expectedTotal)} tool operations hit`,
  );
  ...
  write(
    `    Coverage >= ${String(expectedTotal)} tools:  ${stats.coveredTools >= expectedTotal ? "PASS" : "FAIL"} (${String(stats.coveredTools)}/${String(expectedTotal)})`,
  );
  write(`    No crashes:            PASS`);
  write("");

  process.exit(
    errorRate < 5 && opsPerSec > 10 && stats.coveredTools >= expectedTotal
      ? 0
      : 1,
  );
}
