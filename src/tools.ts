...
const CONSOLIDATED_PRESETS: Record<string, readonly string[]> = {
  ...
  minimal: ["vault", "search", "status", "configure"],
  safe: [
    "vault",
    "active_file",
    "commands",
 ...
 */
const PROTECTED_GRANULAR = new Set([
  "configure",
  "get_server_status",
  "refresh_cache",
]);
const PROTECTED_CONSOLIDATED = new Set([
  "configure",
  "status",
  "vault_analysis",
]);
...
export function registerAllTools(
  ...
  const protectedTools = isConsolidated
    ? PROTECTED_CONSOLIDATED
    : PROTECTED_GRANULAR;
  const preset = presets[config.toolPreset];
  if (!preset) {
    // toolPreset is validated in config.ts — this is a defensive guard
    throw new Error(
      `Unknown tool preset: "${config.toolPreset}". Valid presets: ${Object.keys(presets).join(", ")}`,
    );
  }

  const shouldRegister = buildFilter(
    preset,
    config.includeTools,
    config.excludeTools,
    protectedTools,
  ...
  if (isConsolidated) {
    return registerConsolidatedTools(
      server,
      client,
      cache,
      shouldRegister,
